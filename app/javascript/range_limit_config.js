...
const configureRangeLimit = function () {
  var config = {
    ...
    $.ajax({
      ...
      configs.forEach(function (conf) {
        if (conf.field === newField) {
          $(
            ".blacklight-" +
              $customDateRange.data().dateRangeSelectorOriginalField
          ).data("plot-config", conf)
        }
      })
      // Select content and insert it
      var $content = $(html).find(".custom-range-limit-container").children()
      $customDateRange
        .find(".custom-range-limit-container")
        .html($content)
        .show()
...
export default configureRangeLimit
